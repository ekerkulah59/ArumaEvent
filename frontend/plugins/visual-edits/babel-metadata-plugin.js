...
const babelMetadataPlugin = ({ types: t }) => {
    ...
    for (const [absPath, cached] of FILE_AST_CACHE) {
      ...
      traverse(ast, {
        ImportDeclaration(importPath) {
          ...
          // Use parentPath (Program) not parentPath.parentPath
  ...
  function getArrayIterationContext(exprPath, state) {
    ...
    const callExprParent = exprPath.findParent((p) => {
      ...
      if (!ARRAY_METHODS.has(callee.property.name)) return false;

      // FIX: Avoid infinite recursion. If we are analyzing the array node (callee.object)
      // or any part of the callee, we should not consider this map call as the context.
      // We only care if we are inside the callback (arguments[0]).
      const calleePath = p.get("callee");
      if (calleePath === exprPath || calleePath.isAncestor(exprPath)) {
        return false;
      }
